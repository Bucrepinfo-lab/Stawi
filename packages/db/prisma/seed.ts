/**
 * Seed a fresh Stawi database with the demo data used by the prototype.
 * Run with: `npm run -w @stawi/db seed` (requires DATABASE_URL).
 *
 * The demo member "Amina" (clerkUserId "user_demo_amina") belongs to all three
 * groups, mirroring the multi-group switcher in the UI.
 */
...
async function main() {
  // Umoja Women Group (Chama) + a liquid-soap business.
  const umoja = await prisma.group.create({
    data: {
      clerkOrgId: 'org_umoja',
  ...
  // Confirmed contributions → capital per member.
  const amounts: Record<string, number> = {
    'Amina Wanjiru': 4_200_000,
    'Joseph Kamau': 3_800_000,
    'Grace Otieno': 3_100_000,
    'David Mwangi': 2_400_000,
  };
  for (const m of umoja.members) {
    await prisma.contribution.create({
      data: {
        groupId: umoja.id,
        membershipId: m.id,
        amountCents: amounts[m.fullName] ?? 0,
        channel: 'MPESA_STK',
        status: 'CONFIRMED',
      },
    });
  ...
  const umojaBiz = await prisma.business.create({
    data: {
      groupId: umoja.id,
  ...
  // Vijana Investment SACCO (no business yet).
  const vijana = await prisma.group.create({
    data: {
      clerkOrgId: 'org_vijana',
  ...
  for (const m of vijana.members) {
    await prisma.contribution.create({
      data: { groupId: vijana.id, membershipId: m.id, amountCents: 6_500_000, channel: 'BANK', status: 'CONFIRMED' },
    });
  }

  // Mama Mboga Hub (Business group) with a food kiosk.
  const mboga = await prisma.group.create({
    data: {
      clerkOrgId: 'org_mboga',
      name: 'Mama Mboga Hub',
      type: 'BUSINESS',
      members: {
        create: [
  ...
  console.log('Seeded:', { umoja: umoja.id, umojaBiz: umojaBiz.id, vijana: vijana.id, mboga: mboga.id });
}
