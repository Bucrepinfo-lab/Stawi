...
const SEED_ENTRIES: LedgerEntry[] = [
  { id: '1', type: 'SALE', description: 'Daily sales', amountCents: 2_845_000, occurredAt: now },
  { id: '2', type: 'PURCHASE', description: 'Restock', amountCents: 1_900_000, occurredAt: now },
  { id: '3', type: 'EXPENSE', description: 'Rent (share)', amountCents: 250_000, occurredAt: now },
  { id: '4', type: 'SALE', description: 'Sales', amountCents: 2_100_000, occurredAt: daysAgo(2) },
  { id: '5', type: 'SALE', description: 'Sales', amountCents: 1_750_000, occurredAt: daysAgo(9) },
];
...
export function Books() {
  ...
  const pnl = useMemo(() => reconcile(entries, period, now), [entries, period]);
  const insights = useMemo(() => classifyStock(SEED_STOCK), []);
  ...
  const taxes = useMemo(
    () =>
      computeBusinessTaxes({
        ...
        vatRegistered: true,
        tourismSector: true, // a food kiosk → catering levy
      }),
    [pnl.salesCents],
  );
  ...
  return (
    <div>
        ...
        {PERIODS.map((p) => (
          <button
            ...
            style={{
              ...
              background: period === p ? 'var(--forest-deep)' : 'transparent',
              color: period === p ? 'var(--bone)' : 'var(--ink-2)',
            }}
          >
            {p}
          </button>
      ...
      {/* P&L */}
      <div style={card}>
        <h2 className="display" style={{ fontWeight: 600, fontSize: 20, textTransform: 'capitalize' }}>
          {period} profit &amp; loss
        </h2>
            ...
            <div key={t.code} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', background: 'var(--bone)', border: '1px solid var(--line)', borderRadius: 11 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--dim)' }}>
                  {t.remitTo} · due {t.dueBy} ·{' '}
                  <a href={t.link} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--sky)', fontWeight: 600 }}>
                    remit ↗
                  </a>
                </div>
              </div>
          ...
          <tbody>
            {insights.map((s) => (
              <tr key={s.id}>
                ...
                <td style={td}>
                  <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 7, background: 'var(--bone-2)', color: MOVE_COLOR[s.movement], textTransform: 'capitalize' }}>
                    {s.movement}
                  </span>
                </td>
                <td style={td}>
                  {s.critical ? (
                    <span style={{ color: 'var(--danger)', fontWeight: 700, fontSize: 12 }}>⚠ Critical</span>
                  ) : s.needsReorder ? (
                    <span style={{ color: 'var(--warn)', fontWeight: 600, fontSize: 12 }}>Low</span>
                  ) : (
                    <span style={{ color: 'var(--good)', fontWeight: 600, fontSize: 12 }}>OK</span>
                  )}
                </td>
              </tr>
            ))}
            ...
            {alerts.map((a) => (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                <span style={{ fontSize: 13 }}>
                  {a.name} — {a.quantity} left {a.supplierName ? `· ${a.supplierName}` : ''}
                </span>
                {a.supplierPhone && (
                  <a href={`tel:${a.supplierPhone}`} style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--forest-deep)', background: 'var(--gold)', padding: '6px 12px', borderRadius: 9, textDecoration: 'none' }}>
                    Call supplier ↗
                  </a>
                )}
              </div>
...
function Stat({ label, value, accent }: { label: string; value: number; accent?: string }) {
  return (
    <div style={{ background: 'var(--bone)', border: '1px solid var(--line)', borderRadius: 12, padding: 14 }}>
      ...
      <div className="mono" style={{ fontSize: 20, fontWeight: 600, color: accent ?? 'var(--forest-deep)', marginTop: 4 }}>
